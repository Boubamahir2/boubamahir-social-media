...
const followUnfollowUser = catchAsyncError(async (req, res, next) => {
  ...
  const userToFollow = await models.User.findById(req.query.id)
    .select("_id followersCount followingCount isPrivate username");
    ...
    if (userToFollow.isPrivate) {
      ...
      if (fcmToken) {
        await sendNotification(
          fcmToken.token,
          {
            ...
            body: `${notificationData.from.username} sent you a follow request.`,
      ...
      if (fcmToken) {
        await sendNotification(
          ...
          {
            title: "New Follower",
            body: `${notificationData.from.username} started following you.`,
            type: "Followers",
            image: notificationData.from.avatar.url,
          }
        );
