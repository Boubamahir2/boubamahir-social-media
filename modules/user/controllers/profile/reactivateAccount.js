...
import { dateUtil, utility } from '../../../../utils/index.js';
...
const reactivateAccount = catchAsyncError(async (req, res, next) => {
  ...
  const user = await models.User.findOne({ email }).select(
    'password accountStatus firstName email'
  );
  ...
  if (otpObj.user.toString() === user._id.toString()) {
    if (
      ...
      return next(
        new ErrorHandler(ResponseMessages.ACCOUNT_ALREADY_ACTIVE, 400)
      );
    }
