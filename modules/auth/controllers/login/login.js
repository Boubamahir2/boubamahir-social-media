...
const login = catchAsyncError(async (req, res, next) => {
  // the user can login with email or his or her username
  const { emailUname, password } = req.body;

  if (!emailUname) {
    return next(
      new ErrorHandler(ResponseMessages.EMAIL_USERNAME_REQUIRED, 400)
    );
  }
  ...
  if (emailUname && validators.validateEmail(emailUname)) {
    ...
    user = await models.User.findOne({ username: emailUname }).select('+password');
